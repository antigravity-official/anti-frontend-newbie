import { DetailModel } from "../../VM/context";

test("DetailModel 속성값 확인", () => {
  const mock = {
    basePrice: 1395.81,
  ...
  const detailModel = new DetailModel(mock);

  expect(detailModel).toEqual({
    cashBuyingPrice: 1424,
    cashSellingPrice: 1369,
    ttBuyingPrice: 1382,
    ttSellingPrice: 1410,
  });
});
